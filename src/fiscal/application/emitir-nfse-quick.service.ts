...
export class EmitirNfseQuickService {
  ...
  async execute(input: { cpfTomador: string; valor: number }) {
  ...
  private buildPayload(
    ...
    input: { cpfTomador: string; valor: number },
  ): EmitirNfseInput {
    ...
    const codigoNacional = process.env.QUICK_NFSE_CODIGO_NACIONAL ?? '171901';
